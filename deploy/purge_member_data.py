"""Удаляет записи Discord-участника из рабочей базы и локальных копий.

Перед запуском бот нужно остановить. Новая копия базы здесь не создаётся,
иначе удаляемые записи сохранились бы ещё в одном файле.
"""
